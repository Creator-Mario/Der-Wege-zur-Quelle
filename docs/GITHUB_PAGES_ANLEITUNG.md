...
3. **Folder**: Wählen Sie `/docs` aus dem zweiten Dropdown (nicht `/root`)
...
│ Branch                                               │
│ ┌─────────────────────┐  ┌────────────┐            │
│ │ copilot/create-working-study-app ▼│  │ /docs ▼   │            │
│ └─────────────────────┘  └────────────┘            │
│                                                      │
│               [Save]                                 │
└─────────────────────────────────────────────────────┘
...
- **Folder**: `/docs` (WICHTIG: Genau `/docs` auswählen)

### ❌ Häufige Fehler:
- Wenn Sie `/root` oder `/ (root)` wählen, wird die App NICHT funktionieren
- Die Dateien sind im `/docs` Ordner, daher muss dieser ausgewählt werden
- Der Branch muss `copilot/create-working-study-app` sein (nicht `main` oder `master`)

### 🔄 Bei Änderungen:
- Wenn Sie Dateien im `/docs` Ordner ändern und committen
...
## Alternative: Lokales Testen (Offline)

Wenn Sie die App lokal testen möchten (ohne GitHub Pages):

1. Gehen Sie zum Ordner: `Der-Wege-zur-Quelle/docs/`
2. Doppelklicken Sie auf `index.html`
3. Die App öffnet sich in Ihrem Standard-Browser

**Die App funktioniert komplett offline!** Sie können den `/docs` Ordner auch auf einen USB-Stick kopieren und überall nutzen.
...
2. Überprüfen Sie, ob der Ordner `/docs` im Branch existiert
